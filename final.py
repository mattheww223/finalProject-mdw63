...
def encode(text):
    ...
    for char in text.lower():  # encode the original message
        if char in convert.keys():  # check that the character is a letter
            ...
            ret += char
    print(cypher)
    #test1 = getTransitions(text, cypher) These lines are extremely concerning.
    #test2 = processWP()  Error is generally 11-12, decode finds errors ~2.
    #print(calcError(test2, test1))
    return ret
    ...
    with open("WarAndPeace.txt", 'r') as file:  # go through War And Peace
        for line in file:
            for char in line.lower():
                ...
                if noCurrent:  # avoid an error since there is no character
...
def decode(text):
    ...
    errors = [100 for x in range(100)]
    errors[0] = calcError(trueTransitions, textTransitions)
    lettertracker = []
    lettertracker.append(letters)
    counter = 0

    for iteration in range(50000):
        swap1 = letters.index(random.choice(letters))
        swap2 = letters.index(random.choice(letters))
        ...
        newTransitions = getTransitions(text, newLetters)
        ...
        if iteration % 1000 == 0:  # every 1000 iterations, store best letters
                                   # found and minimum error, then restart
            counter = iteration//1000
            letters = letters[:26]
            lettertracker.append(letters)
